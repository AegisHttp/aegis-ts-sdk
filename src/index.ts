...
class AegisHttpSDK {
     ...
     */
    private getOSInfo(): { name: string; cmd?: string; link?: string } {
    ...
    private showInstallDialog(extensionInstalled: boolean = false) {
        ...
        if (osInfo.name === "Windows") defaultVal = "windows";
        else if (osInfo.name === "macOS") defaultVal = "macos";
        else if (osInfo.name === "Debian") defaultVal = "debian";
        ...
        osSelect.onchange = (e) => {
            updateContent((e.target as HTMLSelectElement).value);
        };
